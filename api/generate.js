...
export default async function handler(req,res){
 ...
 try{
  const b=req.body||{};
  const {image,images=[],avatarImage,sceneImage,style,scene,dressCode='Formal / smart',cameraAngle='Closer view',mode='preview',personCount,adjustments=''}=b;
  ...
  const formImages=(extra,background)=>{const f=new FormData();if(background){const a=parse(background);f.append('image[]',new Blob([a.buffer],{type:a.mime}),'locked-scene.png')}else{sources.forEach((src,i)=>{const a=parse(src);f.append('image[]',new Blob([a.buffer],{type:a.mime}),`source-${i+1}.jpg`)})}if(extra){const a=parse(extra);f.append('image[]',new Blob([a.buffer],{type:a.mime}),'avatars.png')}return f};
  if(mode==='avatars'){
  ...
  if(mode==='scene'){
   ...
   const p=`Create ONLY the empty mansion environment for a personalised art print. ENVIRONMENT: ${scenes[sceneKey]}. CAMERA ANGLE: ${cameraAngles[cameraKey]}. The room must be photorealistic like a premium architectural/interior photograph, with physically believable materials, realistic lighting, reflections, depth and shadows. This is a LOCKED BACKGROUND PLATE. Do not include any people, animals, characters, text, logos or watermarks. Do not create an illustration or cartoon room. Compose the room so people can later be placed naturally into it.`;
   const f=new FormData();f.append('model','gpt-image-2');f.append('prompt',p);f.append('size','1024x1024');f.append('quality','low');f.append('output_format','png');
   const r=await fetch('https://api.openai.com/v1/images/generations',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-image-2',prompt:p,size:'1024x1024',quality:'low',output_format:'png'})});
   const d=await r.json();if(!r.ok)return res.status(r.status).json({error:d?.error?.message||'Scene generation failed.'});
   return res.status(200).json({image:`data:image/png;base64,${d.data?.[0]?.b64_json||''}`,mode,scene:sceneKey,cameraAngle:cameraKey});
  }
  if(!sceneKey)return res.status(400).json({error:'Please choose a scene.'});
  const cameraKey=Object.keys(cameraAngles).includes(cameraAngle)?cameraAngle:'Closer view';
  const dress={
   ...
   'Gangster':'dress the characters in stylish 1920s–1930s vintage film-noir-inspired formalwear: tailored suits, waistcoats, smart shirts, suspenders, elegant dresses, period-inspired coats and tasteful accessories where appropriate. Vary the styling naturally by person; hats are optional and should not be used on everyone. Keep the look sophisticated and fashion-focused, with no weapons, violence, criminal activity or threatening behaviour.',
