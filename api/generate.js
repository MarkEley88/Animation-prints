...
export default async function handler(req,res){
 ...
 try{
  const b=req.body||{};
  const {image,images=[],avatarImage,style,scene,dressCode='Formal / smart',mode='preview',personCount,adjustments=''}=b;
  ...
  return res.status(200).json({image:`data:image/png;base64,${d.data?.[0]?.b64_json||''}`,mode,people:count,scene:sceneKey,dressCode});
