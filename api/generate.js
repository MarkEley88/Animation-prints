export default async function handler(req, res) {
  ...
  try {
    const body = req.body || {};
    const { image, images = [], avatarImage, roomImage, style, scene, setting = 'Default', mode = 'preview', personCount, adjustments = '' } = body;
    ...
    if(mode==='analyse'){
      ...
      let people=0,pets=0,notes=[];
      for(const source of sourceImages){
        const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5.6-luna',input:[{role:'user',content:[{type:'input_text',text:'Analyse this customer photo. Count distinct people who are actually visible and real pets. Do not invent anyone. Return ONLY JSON: {"people":number,"pets":number,"notes":"brief neutral identity description"}. Count children and adults.'},{type:'input_image',image_url:source}]}],max_output_tokens:300})});
        ...
        people+=Math.max(0,Number(parsed.people)||0);pets+=Math.max(0,Number(parsed.pets)||0);if(parsed.notes)notes.push(String(parsed.notes));
      }
      return res.status(200).json({people:Math.max(1,Math.min(5,people||1)),pets,notes:notes.join(' ')});
    }
    ...
    if(mode==='avatars'){
      ...
      const form=new FormData();form.append('model','gpt-image-2');sourceImages.forEach((src,i)=>{const a=parseImage(src);form.append('image',new Blob([a.buffer],{type:a.mime}),`source-${i+1}.png`);});form.append('prompt',prompt);form.append('size','1024x1024');form.append('quality','low');form.append('output_format','png');
